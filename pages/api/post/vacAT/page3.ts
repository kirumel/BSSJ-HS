...
export default async function handler(req: any, res: any) {
 ...
 if (req.method === "PATCH") {
    ...
    try {
      ...
      if (findDB.length !== 0) {
        if (findDB.some((a) => a.startTime)) {
          res.status(202).json({ message: "이미 입장 완료된 학생입니다" });
        } else {
          ...
          if (updateDB.count > 0) {
            // 변경된 행 수 확인
            res.status(200).json({
              message: `${findDB[0].name} 학생 입장 완료되었습니다 ${req.body.outTime}`,
            });
          } else {
        ...
        res.status(202).json({ message: "방학 미참여 학생입니다" });
      }
    } catch (error) {
