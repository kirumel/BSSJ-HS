...
interface Student {
  ...
  name?: string;
  comment?: string;
  check?: string; // "1": 출석, "0": 미출석(사유 입력 후), "2": 1차 출석에서 미출석 기록(2차 출석에서 건들지 않음)
  grade?: number;
  class?: number;
}
...
export default function Page({
  ...
  const filteredStudents = selectedClass
    ...
    : [];
  console.log(filteredStudents, tempSelection, bulkAbsentSelection);
  ...
  // 학생 클릭: 일반 모드에서는 tempSelection, 미출석 모드에서는 bulkAbsentSelection에 토글 (id 기준)
  const handleStudentClick = (student: Student) => {
    // 이미 처리된 학생(출석, 미출석, 1차 미출석)은 다시 선택 불가
  ...
  const isStudentDisabled = (student: Student) => {
    // 이미 처리된 경우 (check가 "1", "0", "2" 또는 comment가 있는 경우)
    const alreadyProcessed = studentsData.find(
      (s) => s.id === student.id && (s.check || s.comment)
    );
  ...
  // 해당 반의 모든 학생이 처리되었는지 확인 (출석, 미출석, 또는 1차 미출석(check "2") 모두 완료로 판단)
  ...
  // 출석완료 섹션에 표시할 학생들 (check가 "1", "0", "2" 모두 포함)
