...
export default function Page() {
  ...
  const deleteSelectedStudents = async () => {
    try {
      await axios
        .post("/api/post/vacdeleteStudent", {
  ...
  const handleSort = async () => {
    ...
    setLoading(true);
    await axios.patch("/api/post/sortVac", students).then((response) => {
      if (response.status === 200) {
        setLoading(false);
          ...
          if (studentToEdit) {
            try {
              ...
              const response = await axios.patch(
                `/api/post/updatevacStudent/${studentToEdit.id}`,
                studentToEdit
              );
              if (response.status === 200) {
